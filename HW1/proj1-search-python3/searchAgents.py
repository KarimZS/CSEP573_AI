...
def cornersHeuristic(state, problem):
    ...
    unvistited_corners = list(set(corners)-set(state[1]))

    current = state[0]
    h=0
    while len(unvistited_corners) is not 0:
        min_dist = 999999
        min_corner = ()
        #print(unvistited_corners)
        for corner in unvistited_corners:
            loc_dist = util.manhattanDistance(current, corner)
        ...
        current = min_corner
        #print(current)
        unvistited_corners.remove(min_corner)
    return h
